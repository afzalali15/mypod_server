...
class ExampleEndpoint extends Endpoint {
  ...
  Future<bool> insertArticle(Session session, Article article) async {
    await Article.insert(session, article);
    return true;
  }

  Future<List<Article>> findArticles(Session session, String keyword) async {
    return await Article.find(
      session,
      where: (article) => article.title.like('%$keyword%'),
    );
  }
}
